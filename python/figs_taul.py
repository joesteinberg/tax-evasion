...
for (i,s,g) in zip(range(2),evasion_type,gama):
    ...
    taul0[i]=results2.taul.values[0]
    
    csv_files=[]
    if(i==0):
        csv_files = [f for f in glob.glob(inpath+"ss1_tauk_0.[0-9][0-9][0-9][0-9][0-9][0-9]_evasion_taul.csv")]
    elif i==1:
        csv_files = [f for f in glob.glob(inpath+"ss1_tauk_0.[0-9][0-9][0-9][0-9][0-9][0-9]_no_evasion_taul.csv")]
    ...
    results2.taul=100*(results2.taul-taul0[i])
    ...
    if(i<2):
        ...
        imaxR_tauk[i] = results2.taul.idxmin()
...
for (i,s,g) in zip(range(2),evasion_type,gama):
    ...
    csv_files=[]
    if(i==0):
        csv_files = [f for f in glob.glob(inpath+"ss1_taua_0.[0-9][0-9][0-9][0-9][0-9][0-9]_evasion_taul.csv")]
    elif i==1:
        csv_files = [f for f in glob.glob(inpath+"ss1_taua_0.[0-9][0-9][0-9][0-9][0-9][0-9]_no_evasion_taul.csv")]
    ...
    if(i<2):
        ...
        imaxR_taua[i] = results2.taul.idxmin()
...
axes.set_xticks([-15,0,15,30,45])
axes.set_xlim(-15,45)
axes.set_title('(b) Labor income tax rate (p.p. chg.)',size=8)
...
plt.savefig('output/fig/fig_app_taul_tauk.pdf',bbox='tight',dpi=300)
...
axes.set_title(r'(b) Labor income tax rate (p.p. chg.)',size=8)
axes.legend(loc='lower left',prop={'size':6})
axes.set_xlim(0,15)
...
plt.tight_layout(pad=0,h_pad=1,w_pad=0)
#fig.subplots_adjust(hspace=0.26,wspace=0.3)
plt.savefig('output/fig/fig_app_taul_taua.pdf',bbox='tight',dpi=300)
