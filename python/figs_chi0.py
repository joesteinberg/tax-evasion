...
for (i,s,g) in zip(range(2),evasion_type,gama):
    ...
    csv_files=[]
    if(i==0):
        csv_files = [f for f in glob.glob(inpath+"ss1_tauk_0.[0-9][0-9][0-9][0-9][0-9][0-9]_evasion_chi0.csv")]
    elif i==1:
...
for (i,s,g) in zip(range(3),evasion_type,gama):
    ...
    csv_files=[]
    if(i==0):
        csv_files = [f for f in glob.glob(inpath+"ss1_taua_0.[0-9][0-9][0-9][0-9][0-9][0-9]_evasion_chi0.csv")]
    elif i==1:
...
evasion_type = ['evasion_chi0','no_evasion']
policy = ['warren','sanders']
...
plt.savefig('output/fig/fig_app_tauk_chi0.pdf',bbox='tight',dpi=300)
...
plt.tight_layout(pad=0,h_pad=1,w_pad=0)
#fig.subplots_adjust(hspace=0.26,wspace=0.3)
plt.savefig('output/fig/fig_app_taua_chi0.pdf',bbox='tight',dpi=300)
