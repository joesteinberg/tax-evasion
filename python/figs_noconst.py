...
for (i,s,g) in zip(range(2),evasion_type,gama):
    ...
    results2=pd.read_csv(inpath+'ss0_%s_noconst.csv'%s)
    results2.approval0 = np.nan
    tauk0[i]=results2.tauk.values[0]
    
    csv_files = [f for f in glob.glob(inpath+"ss1_tauk_0.[0-9][0-9][0-9][0-9][0-9][0-9]_"+s+"_noconst.csv")]
...
for (i,s,g) in zip(range(2),evasion_type,gama):
    ...
    results2.approval0 = np.nan    
    csv_files = [f for f in glob.glob(inpath+"ss1_taua_0.[0-9][0-9][0-9][0-9][0-9][0-9]_"+s+"_noconst.csv")]
...
for (i,e,g) in zip(range(2),evasion_type,gama):
    ...
    results2=pd.read_csv(inpath+'ss0_%s_noconst.csv'%e)

    for p in policy:
        try:
            results2 = results2.append(pd.read_csv(inpath+'ss1_%s_%s_noconst.csv'%(p,e)),ignore_index=True)
        except:
            results2 = results2.append(pd.read_csv(inpath+'ss0_%s_noconst.csv'%e),ignore_index=True)
...
plt.savefig('output/fig/fig_app_tauk_noconst.pdf',bbox='tight',dpi=300)
...
axes.set_title(r'(c) Top 0.1\% share',size=8)
axes.legend(loc='upper left',prop={'size':6})
axes.set_xlim(0,8)
...
plt.tight_layout(pad=0,h_pad=1,w_pad=0)
#fig.subplots_adjust(hspace=0.26,wspace=0.3)
plt.savefig('output/fig/fig_app_taua_noconst.pdf',bbox='tight',dpi=300)
